...
function getMapDefaultLocation(location) {
    ...
    mapboxClient.geocoding.forwardGeocode({
        ...
        .then((response) => {
            ...
            const notadjustedDefaultLocationCenter = response.body.features[0];

            console.log("notadjustedDefaultLocationCenter ORIGIN", notadjustedDefaultLocationCenter.center);
            ...
            .then((response) => {
                ...
                let nextJourneyZoomLevel = 1.8;

                const origin = notadjustedDefaultLocationCenter.center;
                const destination = pathToLocation.center;

                console.log("notadjustedDefaultLocation", notadjustedDefaultLocationCenter.center);

                var midpoint = [];
                midpoint[0] = Math.abs(notadjustedDefaultLocationCenter.center[0])-Math.abs(pathToLocation.center[0]);
                midpoint[1] = Math.abs(notadjustedDefaultLocationCenter.center[1])-Math.abs(pathToLocation.center[1]);
