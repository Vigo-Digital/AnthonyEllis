...
function getGeoData() {
    ...
    listLocations.forEach(function(location) {
        ...
        if (found == 0) {
          ...
          filteredGalleries = removeGalleryFromSlider(locationID, country, allGalleries);
...
function removeGalleryFromSlider(id, country, allGalleries) {
...
function getRegionName(lat, long) {
  var searchLatLong = [long,lat];
  //console.log("looking up", searchLatLong);
  mapboxClient.geocoding.reverseGeocode({
      ...
      .then((response) => {
          ...
          //console.log("response features ", response.body.features[0]);
...
function addMapPinLocation(location, lid, lif) {
    ...
    mapboxClient.geocoding.forwardGeocode({
        ...
        .then((response) => {
            ...
            console.log("BEFORE mapLocations.features", mapLocations.features);
            var found = 0
            mapLocations.features.forEach((countryProp) => {
              if (countryProp.properties.id == geoData.properties.id) {
                found++;
              }
              console.log("found location", countryProp.properties.id + ":" + found);
            });

            if (found < 1) {
                mapLocations.features.push(geoData);
            }
            console.log("AFTER mapLocations.features", mapLocations.features);
        });
}
...
map.on("load", function(e) {
      ...
      input.addEventListener("input", function(event) {
        ...
        for (var member in mapLocations.features) delete mapLocations.features[member];
