...
function removeGalleriesFromSlider(rangeYearFrom, rangeYearTo) {
    ...
    mapLocations.features.forEach((location) => {
        ...
        console.log("check", rangeYearFrom + ">=" + separatedgalleryYearsArray[0] + "&&" + toGalleryYear + "<=" + separatedgalleryYearsArray[actualMaxYearItem]);
        console.log("--------");

        if ((rangeYearFrom >= separatedgalleryYearsArray[0]) && (toGalleryYear <= separatedgalleryYearsArray[actualMaxYearItem]))
        ) {
            removeGalleryCount++;
