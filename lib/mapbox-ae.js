  ...
  map.addLayer({
    ...
    // When the user moves their mouse over the page, we look for features
            ...
            map.on("mousemove", function(e) {
              ...
              //console.log("map.queryRenderedFeatures(e.point)", map.queryRenderedFeatures(e.point));

              const continentDetails = map.queryRenderedFeatures(e.point);
              const displayProperties = [ 'properties', 'sourceLayer' ]
              const displayFeatures = features.map((feat) => {
                const displayFeat = {};
                displayProperties.forEach((prop) => {
                displayFeat[prop] = feat[prop];
                console.log("feat[prop]", feat[prop]);
                });
                ...
                if (activeContinent != "") {
                  ...
                  var new_Filter = ["match",["get","region"],[activeContinent],false,true];
