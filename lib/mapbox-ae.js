...
function getGeoData() {
    ...
    listLocations.forEach(function(location) {
        ...
        while ((i < separatedArray.length) && (found == 0)) {
            ...
            var eachYearRange = [];
            for (var eyRi = rangeYearFrom; eyRi <= rangeYearTo; eyRi++) {
                eachYearRange.push(eyRi);
            }

            //console.log("eachYearRange.length", eachYearRange.length);

            eachYearRange.forEach(checkGalleryYear);

            function checkGalleryYear(eachYearItem) {
                if(eachYearItem == separatedArray[i]){
                    //console.log("found!!!!!!");
                    found++;
...
function removeGalleriesFromSlider(rangeYearFrom, rangeYearTo) {
    ...
    mapLocations.features.forEach((location) => {
        ...
        let separatedgalleryYearsArray = location.properties.years;
        separatedgalleryYearsArray.sort();
        let amountofGalleryYears = Object.keys(separatedgalleryYearsArray);

        //console.log("location", location.properties.id);
        //console.log("separatedgalleryYearsArray", separatedgalleryYearsArray);
        //console.log("amountofGalleryYears", amountofGalleryYears);
       //console.log("rangeYearFrom", rangeYearFrom);
        //console.log("rangeYearTo", rangeYearTo);

        var eachYearRangeRemove = [];
        for (var eyRRi = rangeYearFrom; eyRRi <= rangeYearTo; eyRRi++) {
            eachYearRangeRemove.push(eyRRi);
        }

        eachYearRangeRemove.forEach(checkGalleryYearRemoval);
        //console.log(separatedgalleryYearsArray);

        function checkGalleryYearRemoval(eachYearItemRemoval) {
            //console.log("new logic check XX4");
            //console.log("does" + String(eachYearItemRemoval) + " exist in " + separatedgalleryYearsArray)
            if(separatedgalleryYearsArray.includes(String(eachYearItemRemoval), 0)){
                //console.log("keep");
        ...
        if (keep <= 0) {
            //console.log("remove location", location.properties.id);
            const child = document.getElementById(location.properties.country);
            if (child) {
                child.parentElement.style.display = "none";
            }
        } else {
            //console.log("re-add location", location.properties.id);
            //console.log("re-add location name check", location.properties.country);
            const child = document.getElementById(location.properties.country);
...
function addMapPinLocation(location, lid, lif, locationyears, locationcountry) {
    //console.log("looking up", typeof(locationyears));
    mapboxClient.geocoding.forwardGeocode({
        ...
        .then((response) => {
            ...
            const locationyearsarray = locationyears.split(',').map(y => y.trim());
...
function addMapPoints(initial = false) {
    ...
    map.on("click", "marker-points", (e) => {
        ...
        if (urlCountry == undefined) {
            urlCountry = e.features[0].properties.country;
        }
        ...
        if(urlCountry == "Costa Rica") {
            urlCountry = "costarica";
        }
        if(urlCountry == "Saint Lucia") {
            urlCountry = "saintlucia";
        }
        if(urlCountry == "Czech Rep.") {
            urlCountry = "czech";
        }
        if(urlCountry == "Czechia.") {
            urlCountry = "czech";
        }
        if(urlCountry == "Myanmar (Burma)") {
            urlCountry = "myanmar";
        }
        if(urlCountry == "Bosnia and Herz.") {
            urlCountry = "bosnia";
        }
        if((urlCountry == "Lao PDR") || (urlCountry == "lao pdr")){
            urlCountry = "laos";
        }
...
map.on("load", function(e) {
      ...
      input.addEventListener("input", function(event) {
        ...
        //map.setFilter('marker-points', ["any", ["all", ["in", ['get', 'years'], ["literal", [eachYearRangeX]]], ["in", ['get', 'years'], ["literal", [eachYearRangeX]]]]]);
        //map.setFilter('marker-points', ["any", ["all", ['>=', ['get', 'years'], rangeYearFrom], ['<=', ['get', 'years'], rangeYearTo]]]);

        const selectedYears = [];
        for (let y = rangeYearFrom; y <= rangeYearTo; y++) {
        selectedYears.push(String(y));
        }

       console.log("Selected Years", selectedYears);

        /*map.setFilter('marker-points', [
            "any",
          ...
          mapLocations.features.forEach((feature) => {
            if (typeof feature.properties.years === "string") {
              try {
                feature.properties.years = JSON.parse(feature.properties.years);
              } catch (e) {
                console.warn("Could not parse years array for:", feature.properties.id);
              }
            }
          });
          /*map.setFilter('marker-points', [
            "any",
            ...selectedYears.map(year => ["in", year, ["get", "years"]])
          ]);*/
          const germanyFeature = mapLocations.features.find(f => f.properties.id === 'germany');
          console.log('Germany years:', germanyFeature?.properties.years);

          map.setFilter('marker-points', [
            "any",
            ...mapLocations.features.flatMap(feature => {
              return feature.properties.years
                .filter(year => selectedYears.includes(year))
                .map(year => ["==", ["get", "id"], feature.properties.id]);
            })
          ]);
            //map.setFilter('marker-points', ["in", "2015", ["get", "years"]]);
    ...
    input2.addEventListener("input", function(event) {
        ...
        const mbfeatures = map.queryRenderedFeatures({ layers: ['marker-points'] });

        mbfeatures.forEach(f => {
            console.log(f.properties);  // See what's inside "years", and its format
        });
        ...
        map.setFilter('marker-points', [
        "any",
        ...selectedYears.map(year => ["in", String(year), ["get", "years"]])
        ]);*/
        ...
        /*map.setFilter('marker-points', [
          ...
          const source = map.getSource('marker-points');
          console.log('Source data:', source._data.features.find(f => f.properties.id === 'germany').properties.years);
            console.log('Type of years:', typeof source._data.features.find(f => f.properties.id === 'germany').properties.years);

          /*const filter = ["any"];
            selectedYears.forEach(year => {
            filter.push(["in", year, ["get", "years"]]);
            });
            map.setFilter("marker-points", filter);*/
            map.setFilter('marker-points', ["in", "2015", ["get", "years"]]);

          console.log(
            "Germany years:",
            mapLocations.features.find((f) => f.properties.id === "germany").properties.years
          );

        //getGeoData();
        //map.setFilter('marker-points', ["any", ["all", ["in", ['get', 'years'], ["literal", [eachYearRangeY]]], ["in", ['get', 'years'], ["literal", [eachYearRangeY]]]]]);
        //map.setFilter('marker-points', ["any", ["all", ['>=', ['get', 'years'], rangeYearFrom], ['<=', ['get', 'years'], rangeYearTo]]]);
        removeGalleriesFromSlider(rangeYearFrom, rangeYearTo);
        //addMapPoints();
    });
        ...
        const displayFeatures = features.map((feat) => {
            ...
            if (activeContinent != "") {
                ...
                if (subRegion != "") {
                    ...
                    //if south america, use countries
        ...
        if (currentZoom > initialZoomLevel) {
            if (features.length) {
                ...
                for (const value of iterator) {
                    ...
                    if (value == "Laos" && features[0].properties.name == "Lao PDR") {
                        foundHover = true;
                    }

                    if (value == "Bosnia" && features[0].properties.name == "Bosnia and Herz.") {
                        foundHover = true;
                    }
                }

                filterName = features[0].properties.name;
                //console.log("hovered country", filterName);
                if (filterName == "N. Cyprus") {
                    filterName = "Cyprus";
                    features[0].properties.name = "Cyprus";
                    foundHover = true;
                }

                if (foundHover) {
                    map.getCanvas().style.cursor = 'pointer';
                    if ((filterName == "N. Cyprus" || filterName == "Cyprus")) {
                        map.setFilter("cfh", ["match", ['get', 'name'], ["Cyprus", "N. Cyprus"], true, false]);
                    } else {
                        map.setFilter("cfh", ["==", "name", filterName]);
                    }
                } else if ((filterName == "Myanmar") || (filterName == "Myanmar (Burma)")) {
                    map.setFilter("cfh", ["==", "name", "Myanmar"]);
                } else {
