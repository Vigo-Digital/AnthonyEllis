...
function addMapPoints(initial = false) {
    ...
    map.on("click", "marker-points", (e) => {
        ...
        var urlCountry = e.features[0].properties.name;

        if(urlCountry == "United Kingdom") {
            urlCountry = "uk";
        }
        window.location.href = window.location.protocol + "//" + window.location.host + '/galleries/' + foundCountryClick;
...
map.on("load", function(e) {
    ...
    map.on("click", "cfh", (e) => {
        ...
        const poppedupLatLong = document.getElementsByClassName("activePopUp");*/
        let urlCountry = "";
        if (mpLoc.properties.country) {
            urlCountry = mpLoc.properties.country;
        } else {
