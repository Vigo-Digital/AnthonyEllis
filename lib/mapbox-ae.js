  ...
  function updateGallerySlider(allGalls) {
    ...
    console.log("map.getStyle().layers", map.getStyle().layers);
		...
		map.on("click", "points", (e) => {
			...
			new mapboxgl.Popup({ className: "activePopUp" } ).setLngLat(coordinates).setHTML(description).addTo(map);
    ...
    // When the user moves their mouse over the page, we look for features
            ...
            map.on("mousemove", function(e) {
              ...
              const displayFeatures = features.map((feat) => {
                ...
                var activeContinent = displayFeat.properties.subregion;
                console.log("Active Continent Hovered", activeContinent);

                if (activeContinent.includes("Europe")) {
                  activeContinent = "'Western Europe', 'Eastern Europe', 'Northern Europe', 'Southern Europe'";
                }

                console.log("Active Continent Combined", activeContinent);


                if (activeContinent != "") {
                  //var filter_map = map.getFilter("Dynamic - Inactive Country Overlay");
                  //console.log("current filter", JSON.stringify(filter_map));
                  var new_Filter = ["match",["get","subregion"],[activeContinent],false,true];
              ...
              //console.log("Water hover", continentDetails[0].sourceLayer);
