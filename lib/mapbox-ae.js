...
function removeGalleriesFromSlider(rangeYearFrom, rangeYearTo) {
    ...
    mapLocations.features.forEach((location) => {
        ...
        if ((rangeYearFrom > separatedgalleryYearsArray[0]) && (rangeYearTo > toGalleryYear)) {
            removeGalleryCount++;
            console.log("We need to remove", location.properties.id);
        } else {
            showGalleryCount++
            console.log("We DONT need to remove", location.properties.id);
        }

    });
    console.log("--------");
        console.log("SHOW", showGalleryCount);
        console.log("REMOVE", removeGalleryCount);
