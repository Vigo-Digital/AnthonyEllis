...
map.on("load", function(e) {
      ...
      input.addEventListener("input", function(event) {
        ...
        const yearFeatures = map.queryRenderedFeatures({ layers: ['marker-points'] });
        if (yearFeatures) {
            const uniqueMarkerYears = getUniqueFeatures(yearFeatures, 'years');
            console.log("uniqueMarkerYears", uniqueMarkerYears);
        }
    ...
    input2.addEventListener("input", function(event) {
        ...
        //getGeoData();
        map.setFilter('points', ['==', ['get', 'years'], rangeYearFrom]);
        //addMapPoints();
    });
