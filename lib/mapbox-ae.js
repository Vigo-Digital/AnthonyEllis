            ...
            if ((rangeYearFrom <= separatedArray[i]) && (rangeYearTo >= separatedArray[i])) {
                ...
                if (found == 1) {
                  if (mapPinLocationOverride != "") {
                      ...
                      if (map.getSource("points")) {
                        console.log("here2", mapLocations);
                        map.getSource('points').setData(mapLocations);
                      }
                  }
                }
...
map.on("load", function(e) {
        ...
        var myFeatureLayer = map.getLayer('points');
        console.log("myFeatureLayer", myFeatureLayer);
        const markerYears = map.queryRenderedFeatures({ layers: ['points'] });
        console.log("markerYears", markerYears)
      ...
      input.addEventListener("input", function(event) {
        ...
        myFeatureLayer.setFilter(function(f) {
            return f.properties['years'].includes(rangeYearFrom);
        });
