...
console.log("listLocations", listLocations);
...
map.on("load", function(e) {
    ...
    map.on("mousemove", function(e) {
        ...
        const displayFeatures = features.map((feat) => {
            ...
            if (activeContinent != "") {
                ...
                setTimeout(function(){
                    map.setLayoutProperty("Dynamic - Inactive Country Overlay", 'visibility', 'visible');
                    map.setPaintProperty('Dynamic - Inactive Country Overlay', 'fill-opacity-transition', {duration: 1000, delay: 0});
                }, 300)
            } else {
        ...
        if (currentZoom > initialZoomLevel) {
            if (features.length) {
                ...
                //console.log("locations", Object.values(mapLocations.features));
                map.getCanvas().style.cursor = 'pointer';
    ...
    map.on("click", function(e) {
        ...
        if (features.length) {
            console.log(e + "here", features[0].properties.name);
        }
    });
});
