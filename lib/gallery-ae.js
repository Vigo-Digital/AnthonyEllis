          ...
          if (figureLB.children.length < 2) {
            altData.forEach((entry) => {
              if (entry._src == imgLB.src) {
                ...
                if (entry._alt)  {
                  //figureLB.prepend(figcaption);
                  let lbView = newMut[0].target.querySelector("w-lightbox-view");
                  lbView.prepend(figcaption);
                }

                //if (entry._alt) lbContent.prepend(figcaption);
              }
            });
          }
        }
