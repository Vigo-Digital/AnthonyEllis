...
allImages.forEach((el) => {
								...
								if (entry._src == imgLB.src) {
									...
									if (figExists == null) {
										...
										if (entry._alt)  {
											...
											figcaption.innerHTML = toTitleCase(boldedLBAlt.bold()) + altLBRemainder.toUpperCase();
										...
										if (entry._alt)  {
											let boldedLBAlt = entry._alt.substring(entry._alt.indexOf("*") + 1, entry._alt.lastIndexOf("*"));
											let altLBRemainder = entry._alt.substring(entry._alt.lastIndexOf("*")+1);
											figExists.innerHTML = toTitleCase(boldedLBAlt.bold()) + altLBRemainder.toUpperCase();
	...
	document.getElementById("Multi-Image-0-Caption-Block").innerHTML = toTitleCase(boldedAlt.bold()) + altRemainder.toUpperCase();
	...
	for (let imgi = 1; imgi < numberofImageElements; imgi++) {
		...
		if (elementToGetAlt != "") {
			...
			if (document.getElementById(multiImageBlockName)) {
				...
				document.getElementById(multiImageBlockName).innerHTML = toTitleCase(boldedAlt.bold()) + altRemainder.toUpperCase();
		 	} else {
				var photoLabelAltContentDiv = document.createElement("div");
				...
				photoLabelAltContentDiv.innerHTML = toTitleCase(boldedAlt.bold()) + altRemainder.toUpperCase();
			}
		}
	}
