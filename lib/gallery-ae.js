...
allImages.forEach((el) => {
  ...
  const lbOpenObserver = new MutationObserver((mut) => {
    mut.forEach((mutation) => {
      ...
      console.log('lightbox open 6');

      var htmlElement = document.querySelector('html');

      if (htmlElement.classList.contains('w-lightbox-noscroll')) {
        console.log('html has lightbox');
      } else {
        console.log('html doesnt have lightbox');
      }
      

      // // Get the body element
      // var body = document.body;
      // if (!body.classList.contains('js-lightbox-open')) {
      //   body.classList.add('js-lightbox-open');
      // } else {
      //   body.classList.remove('js-lightbox-open');
      // }
