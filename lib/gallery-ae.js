	...
	const introText = document.getElementsByClassName("paragraph-2");
	var introTextHtml = introText[0].innerHTML;
	//console.log("introTextHtml", introTextHtml);
	var newIntro = introTextHtml.replaceAll('&gt;&gt;', '<br />');
	//console.log("newIntro", newIntro);
	introText[0].innerHTML = newIntro;


	const introNewParaText = document.getElementsByClassName("newpara");
	var introNewParaTextHtml = introNewParaText[0].innerHTML;
	var newParaIntro = introNewParaTextHtml.replaceAll('&gt;&gt;', '<br />');
	introNewParaText[0].innerHTML = newParaIntro;
	...
	//console.log("textBlock1", textBlock1);
	var newTextBlock1 = textBlock1.replaceAll('&gt;&gt;', '<br />');

	if (textBlock1 != "") {
		document.getElementById("Text-Block-Replace-1").innerHTML = newTextBlock1;
	} else {
	...
	function getAndCheckAllPortalsTwo(caption) {
		...
		var portalExitGallery = document.querySelectorAll(".portal-gallery-exit");
		var portalExitImage = document.querySelectorAll(".portal-exit-image");

		//console.log("portalExitGallery", portalExitGallery);
		//console.log("portalExitImage", portalExitImage);

		var allPortals = new Array();
		for (portali = 0; portali < portals.length; portali++) {
			var portalDetail = {"Alt":portals[portali].innerHTML, "Gallery":portalExitGallery[portali].innerHTML, "Slug":portalSlugs[portali].innerHTML, "ExitGallery":portalExitGallery[portali].innerHTML, "ExitImage":portalExitImage[portali].innerHTML};
		...
		//console.log("dupePortals", dupePortals);

		allPortals.forEach((portal) => {
			//console.log("Portal caption is", portal.Alt);
			//console.log("Looking for", caption);

			if(portal.Alt.toUpperCase() == caption.toUpperCase()) {
				console.log("FOUND PORTAL");
				matchedAlt = true;
				console.log("The slug you want is", portal.Alt);
				//sluggy = portal.Slug;
				sluggy = portal.Alt.toUpperCase();
			}
			portalNum++;
		});
		...
		if (matchedAlt) {
			...
			allPortals.forEach((portal) => {
				...
				console.log("Try matching portal", portal.Alt.toUpperCase());
				console.log("against", sluggy);
				console.log("portalIndex", portalIndex);

				/*
				if (portal.portalExitImage == sluggy) {
					console.log("Matched Slug");
					console.log("portalIndex", portalIndex);
				} else {
					portalIndex++;
				}*/
				if (portal.Alt.toUpperCase() == sluggy) {
					console.log("found!");
					console.log("portalIndex", portalIndex);
					gotoPortal = dupePortals[portalIndex];
				}
				portalIndex++;
			});

			console.log("gotoPortal", gotoPortal);
			return gotoPortal;
		} else {
			return false;
		}
...
allImages.forEach((el) => {
	...
	function createTextBlock(position, style, number) {
		//console.log("Create Text Block inside");
			...
			if (document.getElementById(elementIdwithNum).innerHTML != "") {
				...
				photoLabelDiv.className = 'photolabel plright';
	...
	let allTextBlocks = document.getElementsByClassName("text-block-10");
	for (let aTB = 0; aTB < allTextBlocks.length; aTB++) {
		let preTextHtml = allTextBlocks[aTB].innerHTML;
		console.log("preTextHtml", preTextHtml);
		let newTextBlock = preTextHtml.replaceAll('&gt;&gt;', '<br />');
		console.log("newTextBlock", newTextBlock);
		allTextBlocks[aTB].innerHTML = newTextBlock;
	}
...
function toTitleCase(str) {
	const cleanText = str.replace(/(<([^>]+)>)/gi, '');

	const newString = cleanText
		// First, lowercase everything (Unicode-safe)
		.toLocaleLowerCase()
		.replace(
			// Match words and apostrophe-names
			/\p{L}+(?:'\p{L}+)?/gu,
			word => {
				// Handle names like O'Neill, D'Artagnan
				if (word.includes("'")) {
					const [first, second] = word.split("'");

					// Possessive: 's → keep lowercase
					if (second === "s") {
						return (
					...
					return (
						...
						first.slice(1) +
						"'" +
						second.charAt(0).toLocaleUpperCase() +
						second.slice(1)
					);
				}
	...
	function createLine(type, id, removeCaption) {
		...
		if (type == "up") {
			...
			if (rowImgLineCaption) {
				rowImgLineCaption.classList.add("uppline");
...
function getLoadedImages() {
	...
	for (let imgis = 1; imgis < numberofActualImageElements; imgis++) {
		...
		if (foundPortal) {
			...
			wrapper.setAttribute('href', "/galleries/" + gallery.replace(/\s+/g, '-').toLowerCase() + "?lightbox=" + foundPortal.ExitImage + "#" + foundPortal.ExitImage);
			wrapper.appendChild(imgElement.cloneNode(true));
			wrapper.id = formatClearedAlt;
